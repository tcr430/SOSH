...
function makeRequest(opts: {
  ...
  return new NextRequest('http://localhost/api/cron/backfill', {
...
const tickSummary = {
  tick: '2026-09-14T10:00:00Z',
  durationMs: 5,
  runId: 'run-1',
  runStatus: 'fetching' as const,
  outcome: 'extracting' as const,
  errorCode: null,
  staleFailed: 0,
  stagingPurged: 0,
  stagedVoiceNulled: 0,
...
beforeEach(() => {
  ...
  vi.mocked(runBackfillTick).mockResolvedValue(tickSummary)
})

// ── Bearer mode tests (CRON_TRIGGER='secret') ─────────────────────────────────

describe('GET /api/cron/backfill — auth (dev)', () => {
...
describe('GET /api/cron/backfill — auth (prod)', () => {
...
describe('GET /api/cron/backfill — response shape', () => {
  it('response includes the tick summary', async () => {
    vi.stubEnv('NODE_ENV', 'development')
    const res = await GET(makeRequest({ devTrigger: true }))
    const body = await res.json()
    expect(body.tick).toMatchObject({
      runId: 'run-1',
      outcome: 'extracting',
      staleFailed: 0,
      stagingPurged: 0,
  ...
  it('returns 200 even when runBackfillTick throws (no 500)', async () => {
    vi.stubEnv('NODE_ENV', 'development')
    vi.mocked(runBackfillTick).mockRejectedValue(new Error('backfill boom'))
    const res = await GET(makeRequest({ devTrigger: true }))
    expect(res.status).toBe(200)
    const body = await res.json()
    expect(body.tick).toHaveProperty('error', 'backfill boom')
  })
})

// ── Method-rejection tests ────────────────────────────────────────────────────

describe('GET /api/cron/backfill — 405 when CRON_TRIGGER=qstash', () => {
...
describe('POST /api/cron/backfill — 405 when CRON_TRIGGER=secret', () => {
...
describe('POST /api/cron/backfill — QStash mode', () => {
  beforeEach(() => {
    mockCronTrigger.value = 'qstash'
  })

  it('POST + valid signature → 200 + runBackfillTick called', async () => {
    mockVerifyQStash.mockResolvedValue(undefined)
    const res = await POST(makeRequest({ method: 'POST', upstashSignature: 'valid-sig' }))
    expect(res.status).toBe(200)
    expect(vi.mocked(runBackfillTick)).toHaveBeenCalledOnce()
  })
  ...
  it('POST + invalid signature → 401 + cron-auth-failure warn with reason=qstash-invalid-signature', async () => {
    ...
    expect(warnLog?.route).toBe('backfill')
