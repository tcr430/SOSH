...
**Updated (Session 30, ADR 0023, G1b.1–G1b.13).** Market-responsive (customer-supplied RSS/Atom feeds) is now built: a second signal source alongside company-originated GitHub releases, gated behind a minority allocation cap (2-of-5 shortlist slots, 1-per-feed) per ADR 0021 §12's second-source override (ADR 0023 §17 Amendment 1, does not travel to a third source). The live-run corpus result is measured honestly, not blended: github precision/recall/dismissMatch 1.000/1.000/1.000 (unchanged v1 bootstrap); market_responsive 0/0, 0/24, 9/16 under the corpus's zero-memory stub-tool condition (`docs/decisions/0023-market-responsive-signal-source.md` §2.8).
- **Evergreen-strategic is NOT built** and remains the one intelligence-doc opportunity type with no session behind it.
