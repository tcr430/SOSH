...
| **30.5-DBTESTS-READINESS-RACE** | `db-tests.yml` failed on three consecutive N2.13/N3 attempts (`608f3839`/`b6580b84`/`be03c917`), always at `vault-update-secret.test.ts`'s three permission tests plus **seven** suites reported entirely invisible: `campaigns-social-accounts-role-policies`, `governed-memory-recency-column`, `learning-report-orphans`, `performance-memory-candidates-expiry`, `post-ai-originals-latest-per-post`, `reissue-invite`, `signals3-triage-atomic` (corrected from an earlier undercount of two — Session 30.5-D, D8, NIT-4). **Diagnosis refined (D8):** the PostgREST access log shows two identical calls succeeding with 204 one second before every subsequent call, including `service_role`'s, returned 503, with `57P03 database system is in recovery mode` at that timestamp and `OOMKilled: false` on the Postgres container — Postgres crashing mid-suite under memory pressure (a cgroup kill of a child backend doesn't set the tracked-PID-1 `OOMKilled` flag), not a pre-test schema-cache readiness gap as originally guessed. Independently confirmed against the live linked Supabase project that `public.vault_update_secret`'s actual grants are correct (ADR 0028 §17.4). | **Session 30.5-D, D8 shipped a first attempt at a fix**: `supabase/config.toml`'s `shared_buffers` (256MB→128MB) and `maintenance_work_mem` (128MB→64MB) lowered to leave headroom for GoTrue's bcrypt-driven memory spikes; `db-tests.yml` gains a backgrounded live memory/cgroup-pressure watch uploaded unconditionally (not just `if: failure()`, since a mid-run cgroup kill may never trip that trap). **Whether this fix actually produces a green run is recorded in D9/current-phase.md, not assumed here.** First `db-tests` promotion attempt (ADR 0015 §5) should still not proceed until a green run is observed. |
