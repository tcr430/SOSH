...
| **31D-BLOCKER1-SEGFAULT** | CI `db-tests` job (`ghcr.io/supabase/postgres:17.6.1.111` + `postgrest v14.5`) | `db-tests` run `34694455123` on `session-30-5-adr-0028` at `15aeb764` failed: the Postgres backend **segfaulted twice** (`signal 11`) while executing `reserve_ai_budget` (SECURITY DEFINER, `RETURNS SETOF`) and `vault_update_secret` (SECURITY DEFINER, scalar) via PostgREST's `rpc()` path, each crash killing every in-flight connection regardless of role (`anon`/`authenticated`/`service_role` all failed identically in the affected test files — not a grants regression, per `docs/reviews/session-31-reviewer.md` D20). **Reproduced 2/2** — `gh run rerun 34694455123 --failed` crashed identically at the same two call sites (byte-for-byte same PostgREST-generated SQL shape both times), confirming this is a deterministic environment defect, not a transient flake. Needs: pin/downgrade the CI Postgres/PostgREST image pair, or file upstream with Supabase/PostgREST re: `LATERAL`-joined `SECURITY DEFINER` RPC calls with named (`:=`) params under a role-switched connection. Blocks the `db-tests` promotion tally (currently 0/3) and, transitively, the four Tier-1 rows in ADR 0024 §7 (`QUAL-COST-CEILING-EXTENDED`, `QUAL-BUDGET-PURPOSE-ISOLATED`, `QUAL-PRO-DAILY-POST-CAP`, `QUAL-SCORE-ERASURE`). | Session 31-D, D20 (2026-09-12) |
...
| **30.5-DBTESTS-READINESS-RACE** | `db-tests.yml` has failed on **four** consecutive N2.13/N3/D9 attempts (`608f3839`/`b6580b84`/`be03c917`/`933a335c`), always at `vault-update-secret.test.ts`'s three permission tests, plus a shifting set of invisible suites (seven named in the first three runs; only two — `post-ai-originals-latest-per-post`, `signals3-triage-atomic` — in the fourth, consistent with a crash whose position in file-processing order varies run to run). **Diagnosis, twice refined:** the first three reds showed `OOMKilled: false` alongside a `57P03 recovery mode` crash, read as memory pressure (a cgroup kill of a child backend doesn't set the tracked-PID-1 `OOMKilled` flag). The **fourth red (D9, run 33998672960, after the memory-tuning fix below) shows a sharper signature that changes the diagnosis**: `LOG: server process (PID 362) was terminated by signal 11: Segmentation fault`, with healthy host memory (`free -h`: 7.8Gi free) and `ExitCode=0` — not resource exhaustion at all. A SIGSEGV in a Postgres backend, coinciding with `vault_update_secret` calls across all four reds, points at a crash inside the `pgsodium`/Vault extension's encryption path, not memory pressure. Independently confirmed against the live linked Supabase project that `public.vault_update_secret`'s actual grants are correct (ADR 0028 §17.4). | **Session 30.5-D, D8 shipped a memory-tuning fix** (`shared_buffers` 256MB→128MB, `maintenance_work_mem` 128MB→64MB, a live memory/cgroup-pressure watch uploaded unconditionally) that **did not resolve the crash** — D9 confirmed a fourth red with the segfault evidence above, proving the memory theory wrong rather than confirming the fix. The tuning may still be legitimate hardening but was not the root cause. **Next step, not yet attempted:** pin or upgrade the `ghcr.io/supabase/postgres` image version to rule out a known `pgsodium`/Vault extension bug — a resource-limit adjustment is not the right next move. First `db-tests` promotion attempt (ADR 0015 §5) should not proceed until a green run is observed. |
