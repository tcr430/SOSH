...
| **30.5-DBTESTS-READINESS-RACE** | `db-tests.yml` fails on three consecutive N2.13 attempts, always at `vault-update-secret.test.ts`'s three permission tests (anon-denied, authenticated-denied, service-role-granted) plus two unrelated suites reported entirely invisible. Container logs show `FATAL: the database system is in recovery mode` right after the "Reset DB" step, and `docker inspect` shows the db container was never OOM-killed — this points at the workflow's wait-for-schema-cache-readiness step not accounting for the auth (GoTrue) service's own post-reset recovery window, not a data or permission defect. Independently confirmed against the live linked Supabase project that `public.vault_update_secret`'s actual grants are correct (ADR 0028 §17.4). | **First `db-tests` promotion attempt** (ADR 0015 §5) should not proceed while this readiness race is unresolved — a required gate that intermittently reds on infra timing, not behaviour, would be worse than the current advisory status. Fix: harden `db-tests.yml`'s "Wait for PostgREST schema cache to warm up" step to also wait for GoTrue/auth readiness post-reset, or add a short retry specifically around the first auth-dependent call after reset. |
