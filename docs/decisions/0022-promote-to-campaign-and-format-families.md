...
**Amendment (Session 29-D, D5, MAJOR-5 / A-9) — this arithmetic was dead on arrival, and is now live.** `generatePostsForCampaign`'s idempotency guard (`lib/campaigns/generate.ts:106-114` pre-fix) counted **any** existing post, not generated ones specifically. A promoted campaign always holds exactly one pre-existing post (this section's own `N + 1`, inserted by `promoteDraftToCampaignCore` before generation can ever run), so that guard classified every promoted campaign as `already_generated` on its very first generation attempt — `activateCampaign` was never reached, this section's arithmetic never executed on a real promoted campaign, and the campaign stayed `awaiting_brief` permanently.

**A-9's ruling:** the guard must count **generated** posts, discriminated by `posts.role IS NOT NULL` — a column already write-once and already set only by this same function (ADR 0017 §3.2) — never by `campaigns.origin`. `promoteDraftToCampaignCore`'s `createPosts` call has never set `role` (it writes the human's own draft, not a brief-routed generation), so the promoted snapshot post is `role IS NULL` by construction and is origin-blind: a future non-generation post-creation path that also leaves `role` NULL gets the same correct treatment for free, without this guard ever inspecting where the campaign came from. The named loser: nothing — the guard's double-generation protection for every non-promoted campaign is unchanged (§11.2's `ACTIVATE-PLANNED-UNCHANGED` byte-identity case still holds), and this section's `N + 1` arithmetic is now exercised on the live path (§11.1's `PROMOTE-GENERATE-ACTIVATE-REACHABLE`, §11.2's `GENERATE-GUARD-ROLE-SCOPED`).
...
### 5.5 NIT-7 (Session 29-D, D2) — `studio_drafts.accepted_revision` gets the same bound as `posts.content`

`accepted_revision` (`text NULL`) is written only by `acceptSuggestion` (`lib/db/studio-drafts.ts`), verbatim alongside `content` in the same UPDATE, and flows unmodified into `post_ai_originals.rendered_content` and `payload.content` on promote (`lib/campaigns/promote.ts:141-142`) whenever it is non-NULL. Before this fix it had no bound of any kind — `acceptSchema`'s `acceptedContent` field (`app/[locale]/(dashboard)/studio/actions.ts`) validated with a bare `z.string().min(1)`, so this was the one write path into a `post_ai_originals` column with a weaker contract than every other producer of comparably-sized text in this ADR.

**Decision: `z.string().min(1).max(5000)`**, mirroring `posts.content`'s established contract (`calendar/actions.ts:48`, `posts/actions.ts:179`, and `promote.ts`'s own `PROMOTE_CONTENT_MAX_CHARS` guard on `draft.content`) rather than inventing a new figure — `accepted_revision` and `content` are written from the exact same `acceptedContent` string in the same call, so a different bound on one than the other would be arbitrary. Applied at `acceptSchema` in `app/[locale]/(dashboard)/studio/actions.ts` — the sole write site for the column, so bounding there closes the gap completely; no second bound is needed at `lib/db/studio-drafts.ts` or at the promote read site.

**No DB CHECK added.** `posts.content` itself carries no CHECK, only the app-layer Zod bound at each of its write sites (§5.1's `[sec-2]` correction) — `accepted_revision` has exactly one producer to defend against (unlike `performance_memory.pattern`'s §5.2 rationale, where a CHECK was added specifically because a second, bound-bypassing RPC caller already exists in embryo). Adding a CHECK here would duplicate an already-complete guarantee for no additional producer it could catch. If a second write path to `accepted_revision` is ever added, this decision should be revisited under the same reasoning as §5.2.

Constraint named `ACCEPTED-REVISION-BOUNDED`, added to §11.2 and §20.1.
...
**Amendment (Session 29-D, D6, MINOR-4 / A-10) — the SOURCING half of §6.3 shipped unbuilt.** §6.3 states `carouselRequested` is "sourced from the brief, the deterministic Tier-0 input that already drives generation." Nothing does that sourcing: `lib/ai/generate-native.ts:106` and `lib/ai/prompts/formats/studio-suggestion.ts:142` both hard-code `false` at their `selectFormatFamily` call sites, and `generate-native.ts:133-134`'s `case 'carousel':` branch **throws** — there is no path from a brief to a `'carousel'` selection today. `CarouselOutputSchema` and `validateCarouselPolicy` accordingly have **no production caller**.

**Ruling: the FAMILY ships; the SOURCING is deferred, on the record, not silently.** §6.3's schema, policy and platform-map rows are real and correctly built — extending `selectFormatFamily`'s signature was the hard, load-bearing part (A-4's reachability argument above), and it is done. What remains is a brief field or equivalent Tier-0 signal that actually sets `carouselRequested` to `true` for some campaign, and no such field exists yet in `CampaignBriefContent` (§2.2). Building one now would mean **re-opening §8.2's frozen Mode 2 prompt fixtures** to prompt for it (the brief-assembly prompt would need to ask the model to signal carousel intent) — exactly the trade §8's `MODE2-PROMPT-BYTE-IDENTICAL` guarantee exists to gate, and out of scope for a correction pass (L-1). **Revival condition, added to §15: the first session explicitly permitted to re-freeze the Mode 2 prompt fixtures, or a brief field that carries the carousel dimension without touching a prompt.**

**Consequence recorded, not hidden:** `CAROUSEL-SCHEMA-STRUCTURAL` and `CAROUSEL-POLICY-SEQUENCE` (§11.2) currently redden over code with **no production caller** — they prove the schema and policy are correct in isolation, not that any live path reaches them. That is now a stated decision (this paragraph), not an unnoticed gap.
...
**Amendment (Session 29-D, D6, MINOR-5 / A-11) — §8.2 wins; §7.1 yields, explicitly.** §7.1 requires `scriptBrief` to be **generated**; §8.2's `MODE2-PROMPT-BYTE-IDENTICAL` forbids changing the Mode 2 prompt fixtures. Nothing reconciled that conflict at build time — `schemas.ts:9-29`'s own source comment records the Builder's engineering call (`.nullish()`, not `imageBrief`'s bare `.nullable()`, precisely because a real model response omits the key entirely: `native-generation-prompt.ts`'s `shapeInstructions` was deliberately left unchanged so `MODE2-PROMPT-BYTE-IDENTICAL`'s frozen fixtures stayed untouched), but a source comment recording an engineering call is not the same as an ADR recording a **ruling**, and this section never made one.

**The Builder's call was correct; what was missing was the ruling, not the code.** §8.2 wins: no prompt is edited by this ADR, in this correction pass, or by any session that has not been explicitly granted permission to re-open the frozen fixtures. §7.1 yields: `scriptBrief` ships as a schema-and-render-ready field (§7.3's rendering, `SCRIPT-BRIEF-BOUNDED`'s bound) that **no production prompt populates today**. `SCRIPT-BRIEF-BOUNDED` therefore currently passes over a field with no writer — a stated decision, not drift. **Revival condition, added to §15 (the same one §6.3's amendment names): the first session explicitly permitted to re-freeze the Mode 2 prompt fixtures, or a brief field that carries script intent without touching a prompt.**
...
| `PROMOTE-RLS-ISOLATED` | The three new columns under the four existing `studio_drafts` policies, **mirrored both directions** (tenant A cannot read or update tenant B's), with `USING` **and** `WITH CHECK` on UPDATE. **Strengthened Session 29-D, D8 (MINOR-3):** a fifth case proves the `WITH CHECK` violation specifically — a tenant updates a row it legitimately sees, attempting to move it into another `business_id`, and is refused. Live-Postgres finding recorded alongside it: loosening `studio_drafts_update_own`'s `WITH CHECK` alone to `true` did NOT let the tunnel succeed — Postgres's RLS for UPDATE also applies the table's `SELECT` policy's `USING` clause to the new row, independently of the UPDATE policy's own `WITH CHECK`. Verified live: loosening BOTH `studio_drafts_update_own.WITH CHECK` and `studio_drafts_select_own.USING` to `true` together let the tunnel succeed (confirmed by a real `business_id` change); restoring either alone re-blocks it. The isolation here is enforced redundantly by both policies. |
| `PROMOTE-MISSING-DRAFT-TYPED` | **Added Session 29-D, D8 (MINOR-8).** A soft-deleted or nonexistent draft, claimed via `claimStudioDraftForPromotion`, returns a typed `{ outcome: 'not_found' }` rather than throwing. Must redden if the fallback re-read's `.maybeSingle()` regresses to `.single()` (verified: reverting reddens both the soft-deleted-draft and the never-existed-draft cases with a thrown "Cannot coerce the result to a single JSON object" error, not a typed result). |
...
| `PROMOTE-GENERATE-ACTIVATE-REACHABLE` | **Added Session 29-D, D5 (MAJOR-5 / A-9).** A promoted campaign driven end to end — `promoteDraftToCampaignCore` → `critiqueBrief` → `approveBriefIfQualified` → `generatePostsForCampaign` — reaches `activateCampaign` and ends `status='active'` with `total_posts_planned` = brief-derived count + 1 (§2.7's amendment), not stuck at `awaiting_brief`. Must redden if the idempotency guard's generated-post filter is removed (verified: reverting to counting all existing posts made `generatePostsForCampaign` return `already_generated` with `postsCreated: 0`, and the campaign never left `awaiting_brief`). |
| `PROMOTE-AI-ORIGINALS-PER-POST-BOUNDED` | **Added Session 29-D, D9 (MINOR-6).** `listLatestPostAiOriginalsByPostIds` returns every requested post's own latest revision, even when one post alone carries more revisions than the old per-LIST cap (`postIds.length * 20`) — proved with one post at 41 revisions and a second at 1, both in a 2-post request. Must redden if `get_latest_post_ai_originals` (`20260825190000_post_ai_originals_latest_per_post.sql`) is reverted to the old post_id-major `ORDER BY` + `LIMIT` shape (verified: reverting made the second post fall off the result entirely — `result.size` 1, not 2). |

### 11.2 Tier 2 — app layer, executed by `app-tests.yml`

`CAROUSEL-SCHEMA-STRUCTURAL` (slide bounds and roles rejected by `safeParse`, not by a downstream string check) · `CAROUSEL-POLICY-SEQUENCE` (`validateCarouselPolicy` distinguishes `policy_violation` from `invalid_response`) · **`MODE2-FORMAT-SELECTION-UNCHANGED`** (§8's `Record<Platform, …>` frozen table) · `MODE2-PROMPT-BYTE-IDENTICAL` (frozen prompt fixtures) · `SCRIPT-BRIEF-BOUNDED` · `PROMOTE-ACTION-VALIDATED` (the Zod contract, including the `max(5000)` copy bound, §5.1) · `PROMOTE-STATES-RENDERED` (all seven §10 states through the real component; Session 29-D, D8 (MINOR-8) adds an eighth, distinct rendered case — `draft_not_found` — beside them, not a renumbering of the seven) · `PROMOTE-CONTRAST-AA` (both themes, reading the shipped token file) · `PROMOTE-I18N-COMPLETE` (en/pt/es) · `ACTIVATE-PLANNED-UNCHANGED` (a non-promoted campaign's `planned` is identical, §2.7) · **`MEM-PATTERN-SENTINEL-GUARDED`** (Session 29-D, MAJOR-1 / A-5 — `upsertDistilledPerformancePattern`'s sole writer boundary applies `neutralizeWithSentinels`, per ADR 0018 Amd A.3, to `insert.pattern` before the RPC call; a mocked-client Tier-2 test asserts a `[/DATA]`-class payload is neutralized before reaching `client.rpc`) · **`MEM-PATTERN-PROMOTER-BOUNDED`** (Session 29-D, MAJOR-2 — `upsertDistilledPerformancePattern` parses `insert.pattern` through a `z.string().max(500)` bound BEFORE the RPC call, mirroring the DB's `performance_memory_pattern_length_check` CHECK at the app layer; two Tier-2 tests assert a 501-char pattern is rejected with `client.rpc` never invoked, and a 500-char pattern at the exact bound is accepted) · **`ACCEPTED-REVISION-BOUNDED`** (Session 29-D, NIT-7 — `acceptSchema`'s `acceptedContent` field, the sole write path into `studio_drafts.accepted_revision`, gains `.max(5000)` mirroring `posts.content`'s established contract; two Tier-2 tests assert a 5001-char value is rejected as `invalid_input` before any DB call, and a 5000-char value at the exact bound is accepted) · **`GENERATE-GUARD-ROLE-SCOPED`** (Session 29-D, D5, MAJOR-5 / A-9 — `generatePostsForCampaign`'s idempotency guard filters existing posts to `role !== null` before counting; one test proves the BYTE-IDENTITY regression case (a non-promoted campaign with a generated post, `role` set, still returns `already_generated`), a second proves the fix (a promoted campaign's `role === null` post does not block generation), and a third proves §2.7's arithmetic on the live path (`activateCampaign` called with generated-count + 1)) · **`CAROUSEL-SLIDE-IMAGE-BRIEF-RENDERED`** (Session 29-D, D9, NIT-5 — `AiOutputPreview.tsx` renders §6.1's per-slide `imageBrief` alongside `role`/`text`, with the same "recommendation, never published" framing and aria treatment §7.3 already establishes for the branch-level field; two Tier-2 tests assert a slide with a real `imageBrief` renders it with the never-published marker in its accessible name, and a slide with `imageBrief: null` renders nothing extra).
...
9. **Carousel's brief-sourcing** (§6.3 amendment, Session 29-D, D6, MINOR-4 / A-10) — `selectFormatFamily`'s `carouselRequested` parameter exists and is correctly wired at both production call sites, but both hard-code `false`; no brief field or other Tier-0 signal sets it to `true` for any campaign. **Revival condition: the first session explicitly permitted to re-freeze the Mode 2 prompt fixtures (§8.2), or a brief field that carries the carousel dimension without touching a prompt.**
10. **`scriptBrief`'s generation** (§7.1 amendment, Session 29-D, D6, MINOR-5 / A-11) — the field is schema-ready, bounded and rendered (§7.3), but no production prompt populates it; §8.2's frozen fixtures win over §7.1's "generated" requirement until revived. **Revival condition: the same one as item 9 — the first session explicitly permitted to re-freeze the Mode 2 prompt fixtures, or a brief field that carries script intent without touching a prompt.**
...
| `PROMOTE-RLS-ISOLATED` | 1 → `db-tests.yml` | Tenant A can read/update tenant B's promote columns, or an UPDATE policy loses its `WITH CHECK`. **Strengthened Session 29-D, D8 (MINOR-3):** a legitimately-visible row can be moved into another tenant via UPDATE. |
| `PROMOTE-MISSING-DRAFT-TYPED` | 1 → `db-tests.yml` | **Added Session 29-D, D8 (MINOR-8).** A soft-deleted or nonexistent draft, claimed for promotion, throws instead of returning a typed `not_found` outcome. |
...
| `PROMOTE-STATES-RENDERED` | 2 → `app-tests.yml` | Any of the seven §10 states stops rendering through the real `PromoteDraftDialog`/`StudioEditor` component. **Extended Session 29-D, D8 (MINOR-8):** the `draft_not_found` case (an eighth, distinct rendered case beside the seven) stops rendering its own message, or collapses into the generic `promote.failed` text. |
...
| `MEM-PATTERN-PROMOTER-BOUNDED` | 2 → `app-tests.yml` | **Added Session 29-D, D2 (MAJOR-2).** `PATTERN_PROMOTER_BOUND_SCHEMA.parse(...)` is removed (or loosened past 500) at `upsertDistilledPerformancePattern` — a 501-char `pattern` reaches `client.rpc` instead of being rejected at the promoter boundary. |
| `ACCEPTED-REVISION-BOUNDED` | 2 → `app-tests.yml` | **Added Session 29-D, D2 (NIT-7).** `acceptSchema`'s `acceptedContent: z.string().min(1).max(5000)` is loosened or removed — a 5001-char value reaches `acceptSuggestion` and is written to `studio_drafts.accepted_revision` instead of being rejected as `invalid_input`. |
| `GENERATE-GUARD-ROLE-SCOPED` | 2 → `app-tests.yml` | **Added Session 29-D, D5 (MAJOR-5 / A-9).** `generatePostsForCampaign`'s guard reverts to counting ALL existing posts instead of `role !== null` ones — a promoted campaign's pre-existing (`role===null`) post is misclassified as `already_generated` again, and/or a non-promoted campaign's generated post stops being classified as `already_generated`. |
| `PROMOTE-GENERATE-ACTIVATE-REACHABLE` | 1 → `db-tests.yml` | **Added Session 29-D, D5 (MAJOR-5 / A-9).** Same guard regression as above, proved end to end against live Postgres — a promoted campaign driven through the real brief/critique/approve/generate chain never reaches `activateCampaign` and stays `awaiting_brief` instead of `active`. |
| `PROMOTE-AI-ORIGINALS-PER-POST-BOUNDED` | 1 → `db-tests.yml` | **Added Session 29-D, D9 (MINOR-6).** `get_latest_post_ai_originals` reverts to a post_id-major `ORDER BY` + list-wide `LIMIT` — a post sorted after another post with many revisions falls off the result entirely instead of getting its own latest revision. |
| `CAROUSEL-SLIDE-IMAGE-BRIEF-RENDERED` | 2 → `app-tests.yml` | **Added Session 29-D, D9 (NIT-5).** `AiOutputPreview.tsx` stops rendering a slide's `imageBrief` when present, or renders it without the never-published marker in its accessible name. |
