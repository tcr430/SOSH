...
1. **Multi-business guard:** `SELECT EXISTS(SELECT 1 FROM businesses WHERE owner_id = $owner LIMIT 1)`. If `true`, the owner still owns another business — **skip** the auth delete and log `auth_user_deleted: false`. (`business_members` shipped in ADR 0013/21A; this guard is unchanged by it — it stays scoped to `businesses.owner_id`, not membership, because `purge_business`'s explicit `business_members` erasure (§8, Rev A/M3) already removes every membership row for the purged business, so a remaining non-owner membership elsewhere never blocks this check.)
