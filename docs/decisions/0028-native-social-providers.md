...
>
> **Correction (Session 30.5-D, D7, NIT-3): the claim above is wrong.** `git log -- docs/launch-checklist.md`
> plus direct enumeration (`grep -n '^\- \[' docs/launch-checklist.md`, lines 456-463) confirm §16 has held
> **exactly eight rows since this session's own N2.11 commit (`231fee86`)** — unchanged since. The build
> guide's original "eight" was correct; this ADR's "seven" was the miscount, and the N3 Reviewer's
> verification table trusted this claim without an independent recount. No row was ever missing or added.
...
| **`launch-checklist.md` §16** | Row 4 corrected (replace, not remove); row count corrected to seven ***(Session 30.5-D, D7, NIT-3: this row-count claim was itself wrong — §16 has held eight rows since N2.11, unchanged; see the §8.3 correction note)*** | §8.3 |
...
1. **A-8: LinkedIn ships MEMBER-ONLY, and this contradicts a locked strategic decision.** CLAUDE.md's locked launch platforms read **"LinkedIn (Business and Founder)"**. With organization posting deferred behind a legal entity that does not yet exist, **launch delivers the Founder half only.** That is a change to a locked decision and needs an explicit founder ruling on one of: launch with LinkedIn Founder-only and say so in customer-facing copy; hold launch for the entity and the Community Management API approval; or amend the locked platform list. **`docs/pre-launch-scope.md` and `docs/product-status.md` must not claim business-page posting until it exists.** *(Superseded context: A-5-prime: the access request is not an engineering task, cannot be started yet, and can fail.)* It is blocked on SOSH having a registered legal entity (§14.1), and once submitted it can still be refused. If Community Management API access is refused or delayed, **LinkedIn ships member-only** and the business-page half of "LinkedIn (Business and Founder)" does not exist at launch. That is a product decision, not a Builder decision — and because it is external and unbounded in time, **it should be treated as the most likely long pole in the whole pre-launch plan.**
   ...
   **Ruled 2026-09-05 (Session 30.5-D, D4, A-9′), superseding the "member-only" framing above as a resolution
   path — the item's underlying fact (Company Page posting is externally gated) is unchanged and this item
   stays open for that reason alone.** The founder rejected staging a member-only launch: *"I want to ship
   with both account types."* The locked platform list is **not** amended, and no customer-facing copy says
   "member-only" or "Founder profile only" — that recommendation is rejected, not implemented. What actually
   ships: (1) dual identity (X, `posts.social_account_id`, `resolvePublishAccount`, the two-identity accounts
   surface) was already built this session (A-6/A-8a) and needs no further work; (2) X delivers both Business
   and Founder identities fully at launch — two OAuth connections against two accounts, no elevated tier, no
   approval; (3) LinkedIn Company Page posting renders `coming_soon` (the A-1 Meta pattern) while the entity +
   Community Management API gate (A-8) holds — an external gate no ruling can lift. A same-day founder
   challenge to A-5-prime's premise was raised and explicitly withdrawn; A-5-prime and A-8 are unchanged and
   unreopened by this note.
...
6. **X's per-post cost versus "unlimited posts" on the Pro plan is an open pricing question** (§14.3), and a founder adjudication. $0.200 per linked post against a €125 plan with no stated ceiling is an unbounded liability; at 10 linked posts/day it consumes roughly half the plan price on one platform alone.
   ...
   **Ruled 2026-09-05 (Session 30.5-D, D4, A-10) — narrowed, not closed.** The ten-linked-posts-a-day figure
   was a worst-case ceiling presented as a forecast; the founder rejected it as unrealistic ("that's just
   noise") — 300 linked posts a month is not a content strategy a human-approval workflow produces. At a
   realistic 2–3 linked posts/day the exposure is **~$12–18/month against a €125 plan, under 15%** — the high
   end, since not every post carries a link. This is recorded as an ordinary COGS line: **no ceiling is set,
   no stated-open pricing decision is opened, and no code-level post cap or throttle is introduced** — the
   founder ruled the exposure immaterial, and capping posts nobody adjudicated would make the product silently
   different from what was sold. N2.1's 2M-vs-3M pay-per-use read-cap discrepancy (§14.3) stays **unresolved**
   as a separate factual gap, not a pricing judgement — cheap to carry open.
7. **Re-authorisation is unavoidable when organization access eventually lands** (A-8). Every LinkedIn user connected under member-only scopes must re-authorise to grant `w_organization_social`. This cannot be engineered away and should be planned as a customer-communication task, not discovered as a support surprise.
8. **§14 may be empty at merge, and that is a stated risk rather than an oversight** (§14.1). A launch-checklist row must require **at least the founder-profile row of §14 filled before launch**; shipping publishing that has never once succeeded against the real API is not a defensible launch state, regardless of how green CI is. *(NIT-2, Session 30.5-D, D7: items 7 and 8 were transposed in the original numbering — "Re-authorisation is unavoidable" and "§14 may be empty" swapped labels here to read in ascending order; neither item's content changed.)*
...
11. **LinkedIn's `r_member_postAnalytics` scope is confirmed review-gated, not shipped** (Session 30.5-D, D4, A-11 — MAJOR-3). Verified against LinkedIn's own "Getting Access to LinkedIn APIs" page: the only tier requiring no special approval ("Open Permissions") lists exactly `profile`, `email`, `w_member_social` — `r_member_postAnalytics` is not among them (`docs/reviews/session-30-5-platform-verification.md` Appendix A item 15, read 2026-09-05). The founder's 2026-09-05 "Yes" authorised adding the scope *if* it proved auto-enabled; it does not, so `PLATFORM_CONFIGS.linkedin.scopes` is unchanged and `saves`/`clicks`/`reach`/`impressions` stay permanently null for LinkedIn until SOSH applies for and receives Community Management API approval for this permission. **Consequence, stated plainly:** this degrades LinkedIn analytics on a Pro tier advertising "advanced analytics" until that approval lands — track the application alongside the one §14.1/Stage 2 already plans for `w_organization_social`.
