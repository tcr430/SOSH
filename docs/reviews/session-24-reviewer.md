...
**Author:** Session 24-D (Claude Code, Sonnet 5). **Date:** 2026-07-24 through 2026-07-25 (D7). **Range
fixed:** `6fcd1ad2^..3f67bcc9` (the eight Session 24 B2.0–B2.7 commits the report above reviews), corrected
via commits on top: `a07c3ef3`/`27991798` (D0), `fc3bb063`/`0bd98d7d` (D1), `98480c0c`/`9e60fd54` (D2),
`397b2c45`/`bb3a0a74` (D3), `f9797d4c`/`fe84ce36` (D4), `ae33672c`/`f50039a1` (D5), `e0809294` (D6),
`93454d94` (D7 — CI-green, BLOCKER-1 closure, PR [#2](https://github.com/tcr430/SOSH/pull/2)). Fixes
recorded here resolve findings in the report above; **nothing above this section is edited** — see
CLAUDE.md REVIEWER-REPORT APPEND-ONLY condition 1.
Everything above the `---` divider preceding this heading is the Reviewer's original text, unedited, dated
2026-07-24 (its own commit); everything from this heading down is Session 24-D's — condition 2's "a reader
must be able to tell, from any line, which of the two wrote it" is satisfied by that single boundary, not
by per-line attribution within this section (all of this section is one author, one pass). Each row cites:
the D-step that resolved it → finding → fix → the test that now proves it → commit SHA.

| Step | Finding | Fix | Test | SHA |
|---|---|---|---|---|
| D7 | **BLOCKER-1** — the range has never executed in CI; no MODE2-\* constraint is yet "covered" (ADR 0015 §2) | Pushed `session-22-d` (B2.0–B2.7 + D0–D6, head `e0809294`) and opened PR #2 against `master`. Both required checks ran green: `app-tests` (tsc + eslint + vitest + skip-guard) in 1m40s; `db-tests` (live Postgres, `supabase/__tests__`) in 2m34s. `db-tests`'s skip-guard (`scripts/ci/assert-no-empty-suite.mjs`) logged `15 file(s) under [supabase/__tests__] all visible, zero failures — green.` — 15 matches the on-disk file count exactly (`ls supabase/__tests__ \| wc -l` = 15), and `mode2-brief-rls.test.ts`/`mode2-role-origin.test.ts` are both in that directory, so both are covered by the guard's per-file invariant: reading the guard's own source confirms it hard-fails with a named `::error::skip-guard: <file> ran zero tests` for ANY file with zero or all-skipped assertions, and it did not fail. **Caveat stated plainly:** the run used `--reporter=json` (suppresses vitest's console per-file test-count summary) and the results JSON is only uploaded as a CI artifact `if: failure()` (`db-tests.yml`) — so the literal numeric executed-count per `mode2-*` file (e.g. "N tests") could not be extracted from this green run with the tools available this session (no local Docker/Supabase CLI to reproduce `test:db` locally either); the guard's fail-loud invariant is what was actually verified, not a raw number. Full detail in `docs/current-phase.md`'s promotion-tally entry, 2026-07-25. | CI itself is the test here — `app-tests` run [30156008250](https://github.com/tcr430/SOSH/actions/runs/30156008250), `db-tests` run [30156008266](https://github.com/tcr430/SOSH/actions/runs/30156008266), both against PR [#2](https://github.com/tcr430/SOSH/pull/2). Every MODE2-\* constraint moves from `AUTHORED-NOT-EXECUTED` to executed; `db-tests` promotion tally recorded as **still 0 of 3** (pull_request-event run, not a `master` run — advisory-but-must-be-read stays in force until 3 consecutive green runs land on `master`). | `93454d94` |
| D0 | **MAJOR-2** — stale edit-state on cross-campaign navigation (missing `key`), page.tsx:40; BriefReviewForm.tsx:37–38 | Added `key={brief.id}` to `<BriefReviewForm>` in `app/[locale]/(dashboard)/campaigns/[id]/brief/page.tsx`. `brief.id` changing forces React to unmount/remount the client component, re-running the `useState(brief.content.narrative)` / `useState(brief.content.proofPlan)` initializers with fresh values — the idiomatic "reset state via key" pattern, not a `useEffect`-sync (which would cause a visible render-then-reset flash). `ecc:react-reviewer` confirmed this is the correct fix, traced `reviseBrief`/`critiqueBrief` and found no edge case where a within-mount reject/edit/re-critique cycle changes `brief.id` while leaving stale local state inconsistent with the row (same-row `UPDATE`, not a new row), and confirmed no other issues in the two files. | New `app/[locale]/(dashboard)/campaigns/[id]/brief/page.test.tsx` (2 tests): asserts the un-rendered `<BriefReviewForm>` element carries `key === brief.id` and `props.campaignId`/`props.brief` match the fetched values, and that the key changes across two calls with different `brief.id`s (the revise/new-brief-row scenario). Follows the existing `app/[locale]/(dashboard)/approvals/page.test.tsx` precedent for reading props off a Server Component's un-rendered element tree (no RTL/jsdom in this repo — `vitest.config.ts` runs `environment: 'node'`). `npx tsc --noEmit --skipLibCheck` clean; `npx vitest run app/ lib/ components/` — 2115/2115 passed (153 files), including the 2 new tests. | `a07c3ef3` |
| D1 | **MAJOR-1** — `getEvidenceMemoryByIds` renders evidence with no tenant scope (citation-by-id boundary asserted, not enforced); MAJOR/BLOCKER severity split (Reviewer: MAJOR, `security-reviewer`: BLOCKER) is now moot — fixed to the stricter standard, without editing either original verdict | Two-part fix, both halves the finding's own "Fix" line called for. **(a) Enforce the boundary:** `getEvidenceMemoryByIds` (`lib/db/memory-evidence.ts`) now takes a required `businessId` param and filters `.eq('business_id', businessId)`, mirroring `listEvidenceMemoryCandidates`; `wrapEvidenceForPrompt` (`lib/ai/wrap-evidence.ts`) threads it through to all three §12 callers — `brief.ts:90` (assembleBrief, `campaign.business_id`), `brief.ts:127→140` (critiqueBrief, `brief.business_id`), `generate-native.ts:95→96` (via a new `businessId` field on `GenerateNativeContentInput`, sourced from `generate.ts`'s function-scoped `businessId` through its `genInput()` closure). **(b) Close the acceptance gap ("optionally" in the original finding, done anyway):** `assembleBrief` now filters the model's returned `pinnedEvidence` to only ids present in `evidenceCandidates` (the ids actually shown to the model) before `createBrief` persists it — an out-of-set id is rejected at the point untrusted model output is first accepted, not just at render time. `security-reviewer` (re-run) confirmed: `brief.business_id` cannot diverge from the owning campaign's `business_id` (`createBrief` derives it server-side from `getCampaignById`, never caller-supplied, and no update path in `campaign-briefs.ts` ever mutates it — traced via `CampaignBriefUpdate`'s field `Omit` in `lib/db/types.ts`); no other service-role `evidence_memory` reader was missed (grepped the tree — `listEvidenceMemoryCandidates` was already scoped, `getEvidenceMemoryByIds` is now the only other reader and is fixed); `businessId` is never attacker-influenceable in this chain (always DB-derived, and `generatePostsForCampaign` additionally cross-checks its `businessId` param against `campaign.business_id` and throws on mismatch); one non-blocking note — the acceptance-gap filter drops silently with no logging, flagged as a future signal worth keeping once the project's logger infra exists (CLAUDE.md: "we'll add this later"), not actioned now. `database-reviewer` confirmed: filter-order in the query chain doesn't affect the generated SQL or plan (Postgres AND-collapses WHERE predicates regardless of `.eq`/`.in` call order); no index covers `(business_id, id)` but none is needed at this cardinality (a handful of pinned ids per brief, PK index on `id` suffices); the empty-`ids` early-return in both functions is intact and unaffected; the per-row `Promise.all` N+1 pattern in `assembleBrief`'s `evidenceCandidates` build is pre-existing (this fix only appends `businessId` to an already-existing one-row-at-a-time call), not worsened — flagged as a possible future NIT (batch into one `getEvidenceMemoryByIds(client, businessId, allIds)` call), out of this fix's scope. | `lib/db/memory-evidence.test.ts`: new dedicated tenancy test (`scopes the read to business_id`) mirroring `listEvidenceMemoryCandidates`'s own pinned tenancy test, plus a foreign-id-renders-nothing test; manually verified to REDDEN when `.eq('business_id', businessId)` is removed from the implementation (reverted after confirming). `lib/ai/wrap-evidence.test.ts`: matching dedicated tenancy test plus a foreign-id-renders-nothing test, proving the param threads all the way through the shared choke point. `lib/ai/generate-native.test.ts`: new test asserting `businessId` reaches `wrapEvidenceForPrompt` unchanged from `GenerateNativeContentInput`. `lib/campaigns/brief.test.ts`: updated the two existing `wrapEvidenceForPrompt` call assertions to include `businessId`; new test asserting a pinnedEvidence id outside the candidate set is stripped before `createBrief` is called (the acceptance-gap close). `npx tsc --noEmit --skipLibCheck` clean; `npx vitest run app/ lib/ components/` — 2121/2121 passed (153 files). | `fc3bb063` |
| D2 | **MINOR-1** — flagship `MODE2-CONTEXT-EQUIVALENT` test ("calls buildCustomerContext with the exact same args as before B2.6", `generate.context-equivalence.test.ts:274–279`) has ZERO `expect()` calls | `vi.spyOn(contextModule, 'buildCustomerContext')` — calls through to the REAL implementation (this file deliberately keeps it unmocked, per the file's own header note), only records invocations — then asserts `expect(spy).toHaveBeenCalledWith(BUSINESS_ID, VARIATION_ID)` and `toHaveBeenCalledTimes(1)`, pinning the pre-B2.6 call shape the test's own name promises. | Same test, now with real assertions. Manually verified to REDDEN: mutated `generate.ts:165`'s call to `buildCustomerContext(businessId, null)`, confirmed the args assertion failed, reverted (`git diff --stat` empty afterward). `pr-test-analyzer` independently re-ran the same mutation and confirmed reddening, and confirmed no other reachable call site inside `generatePostsForCampaign`'s path calls `buildCustomerContext` (the two `brief.ts` call sites are unreached because `getBriefByCampaign` is mocked to return an already-approved brief). `npx tsc --noEmit --skipLibCheck` clean; `npx vitest run app/ lib/ components/` — 2122/2122 passed. | `98480c0c` |
| D2 | **MINOR-2** — regeneration scenario (`generate.test.ts`, hook Tier-2 loop, 7 native calls) never pinned the trial-counter increment against a double-count risk (increment-by-native-call-count vs. increment-by-posts-created) | Added `expect(incrementPostsGeneratedBy).toHaveBeenCalledWith(BUSINESS_ID, 6)` to the existing "regenerates EXACTLY ONCE..." test, right after its existing `toHaveBeenCalledTimes(7)` assertion on `generateNativeContent` — pinning that the increment tracks `postsCreated` (`= inserted.length`, `generate.ts:355/377`, 6 — one row per roleSequence entry) and NOT the native-call count (7, which would double-count the one regenerated post). | Same test, extended. Manually verified to REDDEN: hardcoded `generate.ts:377`'s increment call to a literal `7`, confirmed both the new assertion AND a pre-existing, unrelated "increments trial counter by postsCreated" test in the success-path describe block both failed, reverted (`git diff --stat` empty afterward). `pr-test-analyzer` independently re-ran the same mutation and confirmed reddening. `npx tsc --noEmit --skipLibCheck` clean; `npx vitest run app/ lib/ components/` — 2122/2122 passed. | `98480c0c` |
| D2 | **MINOR-6** — `checkRoleCoverage` (`lib/campaigns/consistency.ts`) passes vacuously (`ok: true`) on an empty `expected` array; no test proved the check catches a REAL coverage gap through the actual orchestrator, only that the pure function is logically self-consistent in isolation | Per the correction-pass instruction's explicit constraint: checked `generate.ts:150–159` first — it already returns early with `error_code: 'invalid_campaign_state'` on `frozenBrief.content.roleSequence.length === 0`, BEFORE the generation loop or `checkRoleCoverage` is ever reached, so the empty-`expected`-vacuous-pass shape is unreachable from the real orchestrator today. Per instruction, the pure function was left UNCHANGED (defense-in-depth, not dead weight) and a new **orchestrator-level** test was added instead, pinning the check's actually-reachable failure mode: a `roleSequence` entry citing a platform outside `CANONICAL_PLATFORM_ORDER` (`'pinterest' as never`) is silently never iterated by the STEP 7 generation loop (`activePlatforms` is built by filtering `CANONICAL_PLATFORM_ORDER` against `roleSequence`), producing a `generated` array one entry short — `checkRoleCoverage` catches it and the run aborts with `consistency_check_failed`, `createPosts` never called. | New test in `generate.test.ts`'s "consistency pass wiring (ADR §8)" describe block. Manually verified to REDDEN: short-circuited `generate.ts:302`'s gate with `if (false && (!roleCoverage.ok \|\| !linkPlacement.ok))`, confirmed both the new test AND the pre-existing tweet-1-link-violation test in the same block failed (`createPosts` got called when it shouldn't), reverted (`git diff --stat` empty afterward). `pr-test-analyzer` independently re-ran the same mutation, confirmed reddening, and separately confirmed the `'pinterest'` scenario is genuinely unreachable through the validated LLM-output path (`ROLE_SEQUENCE_ENTRY_SCHEMA`'s zod enum in `lib/ai/prompts/brief.ts` and `lib/db/types.ts`'s `Platform` union both cover only the 5 canonical platforms) — reachable only via raw-SQL/migration-drift JSONB corruption (no DB CHECK constraint enforces the enum), which the test's own comment already frames honestly as a "future refactor safety net," not a normal-operation case. **Dissent recorded, not actioned:** `silent-failure-hunter` separately argued `checkRoleCoverage` should be hardened to return `ok: false` (or throw) on empty `expected` regardless of any single caller's behavior, since the function's own doc comment claims caller-independence ("independently testable... regardless of whether generate.ts's own control flow can currently produce that state") and a future second call site without generate.ts's upstream guard would silently inherit the vacuous pass. This is a legitimate future-proofing argument, but the correction-pass instruction explicitly pre-empted it: "if [the upstream guard] does [fail first], note the empty-set branch is defence-in-depth and leave it, but pin the reachable failure" — which is what was done. Left for a future session to weigh, not silently dropped. `npx tsc --noEmit --skipLibCheck` clean; `npx vitest run app/ lib/ components/` — 2122/2122 passed. | `98480c0c` |
| D3 | **MINOR-3** (doc-rot) — `generate-native.ts:82–88`'s comment still said the trial double-count "will" happen and was "Flagged for B2.6 to resolve; not addressed here," but B2.6 already resolved it in `runner.ts:13–21` (`NATIVE_GENERATION_PROMPT_IDS` joining the batch-tracked set) | Rewrote the comment to state the CURRENT fact — both native-generation prompt ids now match `isPostGeneration` in `runner.ts`, so the runner skips its per-call increment and only `generate.ts`'s STEP 11 batch increment counts each post — and point at the exact runner.ts lines that fixed it, so a future session reading this comment doesn't reintroduce the bug believing it's still open. | Comment-only change, no behavior to test; `npx tsc --noEmit --skipLibCheck` clean confirms no code impact. | `397b2c45` |
| D3 | **MINOR-4** — `critique` silently dropped when `approveBriefIfQualified`'s refusal result (`{ approved: false; overallScore; critique }`, `brief.ts:179`) is mapped into the Server Action's `ApproveBriefState`; only `overallScore` survived | Added `critique: Record<string, unknown> \| null` to `ApproveBriefState`'s `gate_refused` variant (`actions.ts:64`) and threaded `result.critique` through at the return site (`actions.ts:85`). `silent-failure-hunter` confirmed: no OTHER drop point exists, and while `BriefReviewForm.tsx` doesn't currently read `approveState.critique` anywhere, the user isn't missing anything in practice — the same critique is already rendered unconditionally from the persisted `brief.critique` DB field whenever a brief can be in `gate_refused` territory (i.e. `critiqued` status); flagged the field as currently plumbed-but-unconsumed, harmless and correctly out of this app-layer-state-threading task's scope. | Updated the existing "is REFUSED below threshold" test's expectation to include `critique: null`; added a NEW dedicated test with a NON-null critique object asserting it survives the round-trip (`{status:'gate_refused', overallScore, critique}` exactly) — can't pass by accident on the always-null case alone. `npx tsc --noEmit --skipLibCheck` clean; `npx vitest run "app/[locale]/(dashboard)/campaigns/[id]/brief/actions.test.ts"` — 14/14 passed. | `397b2c45` |
| D3 | **MINOR-7** (guard-strength) — the hook Tier-2 loop's opener-rescoring call (`generate.ts:253–258`) fed the model's own prior output (`extractOpener(output)`) into a second AI call (`rubricPrompt`) relying only on `rubric.ts`'s weaker local ASCII-literal `sanitizeDataField`, not the stronger exported `neutralize()` used elsewhere for reused AI-generated text (L-9 posture) | Wrapped the call site: `content: neutralize(extractOpener(output))`. `silent-failure-hunter` confirmed: `neutralize()` is pure synchronous string manipulation with no throw path under normal execution, so the "guard throws + caller's hook-loop catch swallows it, silently skipping scoring" combination raised as a risk is theoretical, not real; and the leading-brace defusal (ZWSP insertion, not deletion) doesn't meaningfully alter what the scoring model reads even for a stylistic opener like `"{Great tool}"`. | Added a dedicated test with a triple-backtick-fenced opener, asserting the rubric call's `content` never contains `` ``` `` raw. Manually verified to REDDEN: reverted the `neutralize()` wrap locally, confirmed the new test failed (raw `` ```json `` reached the rubric call), re-applied, confirmed green again. The pre-existing plain-ASCII "HOOK-TEXT-MARKER" test still passes unchanged, since `neutralize()` is a no-op on text with no special characters. `npx tsc --noEmit --skipLibCheck` clean; `npx vitest run lib/campaigns/generate.test.ts` — 26/26 passed. | `397b2c45` |
| D3 | **MINOR-8** — `lib/campaigns/brief.ts`'s `content as CampaignBriefContent` cast (superfluous, papering over a possible future zod/DB-type divergence) | Removed the cast; `content` (typed `CampaignBriefContentOutput`, the zod-inferred type) is assigned directly into the `CampaignBriefContent`-typed `sanitizedContent` object literal. `typescript-reviewer` confirmed field-by-field structural identity between `CampaignBriefContentOutput` and `CampaignBriefContent` (including `note?` optionality and the `role`/`platform` literal-union enums matching exactly), confirmed `npx tsc --noEmit --skipLibCheck` stays clean post-removal (no drift existed to catch), and added a precision note: the cast never protected against RUNTIME-refinement drift (e.g. relaxing a `.min(1)`, which doesn't affect the inferred static type either way) — only against TYPE-SHAPE drift (a renamed/added/removed field), which is the real, accurate improvement here. | No new test needed — this is a type-level-only change; `tsc` passing clean IS the proof (a divergence would have been a compile error, per the instruction's own STOP condition, which did not trigger). `npx vitest run app/ lib/ components/` — 2124/2124 passed (153 files), confirming no runtime regression. | `397b2c45` |
| D4 | **MINOR-5** — `FrozenBrief` brand is convention-strength (not nominal — a single-step `as FrozenBrief` is legal TypeScript), and its "cannot be constructed without freezeBrief" comment overstated what TS enforces; separately, `content: Readonly<CampaignBriefContent>` was shallow — arrays stayed typed-mutable, so TS permitted a `.push()` the runtime `Object.freeze` (deepFreezeContent) already threw on | Added a local generic `DeepReadonly<T>` mapped type (recurses into `ReadonlyArray<infer U>` and objects) and retyped `content: DeepReadonly<CampaignBriefContent>`; `deepFreezeContent`'s return type updated to match, so the type now states exactly what the runtime already guaranteed — no behavior change. Softened the doc comment: `freezeBrief` is empirically the only producer today (git grep confirmed), not something the string-literal `_brand` tag nominally enforces; the real backstop against bad data is the DB-layer `frozen_at` guard trigger (B2.0), stated plainly instead of claimed as a type-system property. Per the instruction's "surface it, don't cast around it" directive, the deep-readonly retype broke one real consumer — `checkRoleCoverage` (`lib/campaigns/consistency.ts`) took a mutable-array `expected` param but was being passed the now-`ReadonlyArray` `frozenBrief.content.roleSequence` at `generate.ts:303`. Fixed by widening `checkRoleCoverage`'s `generated`/`expected` params to `ReadonlyArray<...>` — the function only ever reads via `.map()`, never mutates, so this is a legitimate strict widening (still accepts the old mutable-array callers too), not a cast-around. `type-design-analyzer` confirmed: the `DeepReadonly<T>` mapped type is homomorphic (correctly preserves `note?`'s optionality rather than forcing it required), the array-branch-before-object-branch ordering is correct so arrays don't fall through to the generic object case, the `checkRoleCoverage` widening is the right pattern for a pure read-only-consuming function, the softened comment now accurately matches both the code and ADR 0017 §5.2/B2.0 with no remaining over- or under-claim, and the outer `Readonly<{...}> & { _brand }` intersection composes cleanly with the newly-nested `DeepReadonly<CampaignBriefContent>` (different levels: outer prevents reassigning `content` itself, inner prevents mutating through it) — one non-blocking, pre-existing (not introduced by this fix) note that `deepFreezeContent` freezes `row.content` in place rather than a copy, out of scope here. | New test in `brief.test.ts`'s `freezeBrief` describe block: `// @ts-expect-error` on a `.push()` call to `frozen.content.roleSequence`, proving TS now rejects it at compile time (not just Object.freeze at runtime, which the adjacent pre-existing test already covered). Manually verified to REDDEN: reverted `content`'s type to shallow `Readonly<CampaignBriefContent>`, confirmed `tsc` reported "Unused '@ts-expect-error' directive" (the test is load-bearing, not decorative), reverted back (`git diff` matches the committed state afterward). `npx tsc --noEmit --skipLibCheck` clean; `npx vitest run app/ lib/ components/` — 2125/2125 passed (153 files). | `f9797d4c` |
| D5 | **NIT-1** — `approved_success`/`rejected_success`/`saved_success` i18n keys existed symmetrically in en/pt/es but were never rendered; `router.refresh()` alone gave no approve/reject/edit feedback | Founder call: (a) wired an inline confirmation, not (b) key removal — the keys were clearly authored for this. Added a `successMessage` const (`BriefReviewForm.tsx`), derived from `approveState`/`rejectState`/`editState`, computed unconditionally before the `switch (brief.status)` early returns so it survives EITHER outcome shape (approve transitions status to `'approved'`/`'generated'`, an early-return branch; reject/edit keep it `'critiqued'`, the main render path) — rendered as `<p role="status" className="text-sm text-emerald-600">` in all three branches. `role="status"` (implicit `aria-live="polite"`) chosen over the file's existing `role="alert"` (reserved for `gate_refused`/error states) — `react-reviewer` confirmed this is the correct a11y choice for non-urgent positive feedback, confirmed no Rules-of-Hooks issue (successMessage is a plain const, hook call order is identical every render), and confirmed the fragment-wrapping of the two early-return branches introduces no reconciliation risk given the component is already `key={brief.id}`-scoped at its call site (D0). | New `BriefReviewForm.test.tsx` (4 tests, first component-render test for this file — follows the existing repo `createRoot`+`act`+`@vitest-environment happy-dom` pattern, e.g. `SignupForm.test.tsx`/`PostCard.test.tsx`, no RTL dep in this repo): submits each of the three forms via `form.requestSubmit()` through React 19's real `action={formAction}` mechanism (not a simulated state injection) and asserts the matching confirmation text renders, plus a negative case (no confirmation before any submission). `react-reviewer` flagged the initial draft's hardcoded double `await Promise.resolve()` as MEDIUM — timing-dependent on an undocumented React-internal microtask-hop count, not a guarantee; `vi.waitFor` was tried as a fix but timed out (real-timer polling doesn't resolve reliably nested inside one `act(async () => ...)` in this happy-dom setup); landed on a `setTimeout(resolve, 0)` macrotask flush instead — the standard "drain every pending microtask regardless of count" trick, verified stable across 3 repeat runs. `npx tsc --noEmit --skipLibCheck` clean; `npx vitest run app/ lib/ components/` — 2129/2129 passed (154 files). | `ae33672c` |
| D5 | **NIT-2** — `BriefReviewForm.tsx:87,89`: `getCritiqueLines(brief.critique)` called twice (once for a `.length > 0` check, once for `.map()`) on the same input | Hoisted to a single `const critiqueLines = getCritiqueLines(brief.critique)`, computed once alongside `successMessage`, both call sites updated to reference it. | No dedicated test needed (pure refactor, no behavior change) — covered by the existing render path exercised in the new `BriefReviewForm.test.tsx` and the unchanged critique-rendering behavior. `npx tsc --noEmit --skipLibCheck` clean; `npx vitest run app/ lib/ components/` — 2129/2129 passed. | `ae33672c` |
| D5 | **NIT-3** — `types.ts:690`: `CampaignPostRole = PostRole` bare alias; harmless and already well-commented per the Reviewer's own finding text, but "arguably unnecessary indirection" | **Chose: KEEP the alias, tightened the comment** (not inline — `CampaignPostRole` has multiple use sites across the brief-content schema and inlining would erase the ADR's own naming distinction, not simplify it). The existing comment already explained the "same value set, different context" rationale; tightened it to explicitly cite **L-5** (the build-guide's own label for "post ROLES are an explicit field assigned at the brief stage") alongside the existing ADR §3.2/§2.2 references, and stated plainly that inlining would erase, not simplify, the distinction the ADR deliberately draws. | Comment-only change, no behavior to test; `npx tsc --noEmit --skipLibCheck` clean confirms no type-shape impact. | `ae33672c` |
| D5 | **NIT-4** — the three brief Server Actions (`actions.ts`, approve/reject/edit) `catch { return { status: 'error', error: 'generic' } }` with no logging; matches the pre-existing `generate-action.ts` convention (no project logger yet, CLAUDE.md), not a regression — tracked, not built, per the instruction's explicit "do NOT invent a logger here" constraint. Same class, noted not fixed: `key={i}` on the critique `<ul>` (`BriefReviewForm.tsx`) | Added a `// TODO(logger): ...` comment at all three `catch` blocks, each citing NIT-4/Session 24-D and the `generate-action.ts` convention it matches, so the gap is discoverable in-code once the logger lands. Added one row to `docs/backlog.md` (`24D-NIT-4-logger`) covering both the three logger TODOs and the `key={i}` note in a single entry (per the instruction's "same for... note it" framing — not a second backlog row). No code behavior change; this finding is recorded as convention-tracked, per the commit-message framing the instruction specified. | No test — this is a documentation/marker-only change (a swallowed generic-error catch has no distinguishing state to assert on; the existing action tests already cover the `'generic'` return value). `npx tsc --noEmit --skipLibCheck` clean; `npx vitest run app/ lib/ components/` — 2129/2129 passed. | `ae33672c` |

### Notes (Session 24-D, D6 — three things easy to lose)

**1. Why D0–D6 run before BLOCKER-1 (D7), not first.** BLOCKER-1's fix is entirely procedural — push the
branch, let CI run, record the run IDs and the `db-tests` executed-test-count. Running it before D0–D5
would certify a range that still had known MAJOR/MINOR/NIT defects in it; CI green on a range with an
unfixed cross-tenant evidence leak (MAJOR-1) or a vacuous flagship test (MINOR-1) would be a false
all-clear. D7 runs LAST because it is meant to certify the *fully corrected* range — every code-level
finding fixed, tested, and reviewed by its named specialist first, so the CI run that finally satisfies
ADR 0015 §2 ("covered = executed green in CI") is checking the range this report actually recommends
merging, not an intermediate one.

**2. The MAJOR-1 severity split is moot, not resolved by picking a side.** The original Reviewer rated
`getEvidenceMemoryByIds`'s missing tenant scope MAJOR; `security-reviewer` (invoked as a specialist within
that same review) rated it BLOCKER. Neither verdict was edited — both stand exactly as written above the
`---` divider. D1 fixed the underlying defect to the **stricter** of the two standards (business_id-enforced
at the query layer, not just citation-by-id), which is why the severity disagreement no longer has any
bearing on whether this ships: whichever verdict a future reader trusts, the fix clears both bars. This is
recorded here, not resolved by silently deciding one reviewer was "right."

**3. `lib/ai/runner.ts` was changed in B2.6 (`07939de5`) — accepted, not a regression to re-flag.** The
Reviewer's own Reality-4 in the report above already documents this as a deviation from the B2.4 "runner
untouched" claim: `NATIVE_GENERATION_PROMPT_IDS` joined the batch-tracked set in `isPostGeneration`, closing
a real trial-counter double-count bug (native-generation calls would otherwise increment the per-call
counter on top of the orchestrator's batch increment). The Reviewer accepted this as correct — bounded,
tested (`runner.test.ts:345-353`), no new prompt-id/re-prompt logic added to the generic runner — and
MINOR-3 (D3) closed the one remaining loose end (a stale comment in `generate-native.ts` that still
described the bug as unresolved). A future session reading either file should not re-open this as a new
finding; it is closed, on the record, in two places now (the Reviewer's Reality-4 above, and MINOR-3's D3
row).
