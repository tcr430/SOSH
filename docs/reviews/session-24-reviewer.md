...
|---|---|---|---|---|
| D7 | **BLOCKER-1** — the range has never executed in CI; no MODE2-\* constraint is yet "covered" (ADR 0015 §2) | *(pending D7)* — push the branch / open the PR, confirm `app-tests` green (tsc + eslint + vitest + skip-guard) **and** `db-tests` green with a non-zero executed count (skip-guard passing on `mode2-brief-rls` + `mode2-role-origin`). | *(pending D7)* — CI run IDs + `db-tests` executed-test-count, recorded in `docs/current-phase.md`. | *(pending D7)* |
