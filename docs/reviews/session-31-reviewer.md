...
|---|---|
| **Finding** | NIT-3 |
| **Fix** | New migration `supabase/migrations/20260912100000_ai_budget_daily_constraint_names.sql` renames the three stale constraints via `ALTER TABLE ... RENAME CONSTRAINT` — `signal_triage_budget_pkey` → `ai_budget_daily_pkey`, `signal_triage_budget_business_id_fkey` → `ai_budget_daily_business_id_fkey`, `signal_triage_budget_reserved_cents_check` → `ai_budget_daily_reserved_units_check`. No definition change — a rename does not alter what a constraint enforces. Taken as a real fix rather than left as accepted cosmetic debt, since the Reviewer flagged it as genuinely cosmetic with no functional risk either way, making a metadata-only rename the lowest-risk way to close it outright. |
| **Proof** | Read `pg_constraint` on the live linked SOSH project (`phdqfrrkbvuuklvbigoh`) BEFORE the fix: confirmed all three stale names present, and the CHECK's actual definition already correct (`reserved_units >= 0`) — only the name was stale. Applied the migration to the live project (metadata-only `RENAME CONSTRAINT`, low risk); re-read `pg_constraint`: all five constraints (the three renamed, plus the two already-correctly-named `ai_budget_daily_business_id_purpose_day_key` and `ai_budget_daily_purpose_check`) now carry consistent names, all definitions unchanged (FK still `ON DELETE CASCADE`, PK still `(id)`, CHECK still `reserved_units >= 0`). Ran `signals3-triage-state.test.ts` + `ai-budget-generation-posts.test.ts` against the live project: 25/25 green. |
| **Reddening** | N/A — a pure metadata rename with no behavior to redden; the constraints' enforcement (verified identical pre/post via `pg_get_constraintdef`) is what matters, and it provably did not change. |
| **Commit** | `4638adef` |

**What this step did NOT touch:** the two already-correctly-named constraints on this table (`ai_budget_daily_business_id_purpose_day_key`, `ai_budget_daily_purpose_check`) — untouched; any constraint definition — all five re-read and confirmed byte-identical in meaning before and after; `reserve_ai_budget`/`reconcile_ai_budget` RPC bodies or their ACLs (D5's job) — unaffected by a constraint rename on the table they operate on.
...
|---|---|
| **Finding** | NIT-4 |
| **Fix** | `lib/memory/performance.test.ts`'s 17 `const client = {} as any` instances (of which the Reviewer's range-diff confirmed 2 were newly added in Session 31; the other 15 pre-date it) are all switched to `{} as unknown as SupabaseClient` — a properly-typed cast rather than an `any` escape hatch, bringing the file into compliance with CLAUDE.md's stated rule (no `any`, narrow through `unknown`) rather than adding a third carve-out to the two CLAUDE.md already names. Fixed all 17, not just the 2 new ones: leaving 15 identical `as any` lines beside 2 differently-typed ones in the same file would be an inconsistency with no benefit, and the fix is mechanical and behavior-preserving (`{}` is still what's passed at runtime — none of these tests ever invoke a real Supabase method on `client`, since `memory-performance`/`post-metrics`/`posts` are all `vi.mock`'d wholesale). The 17 now-stale `// eslint-disable-next-line @typescript-eslint/no-explicit-any` comments above each cast are removed along with the `any` they were suppressing a warning for. |
| **Proof** | `npx tsc --noEmit --skipLibCheck`: clean. `npx eslint lib/memory/performance.test.ts`: clean (no unused-disable-directive warning, confirming the removed comments were genuinely no longer needed). `grep -c "as unknown as SupabaseClient"` / `grep -c " as any"`: 17 / 0. `performance.test.ts`: 18/18 green, unchanged. |
| **Reddening** | N/A — a compile-time type-safety fix with an identical runtime value (`{}` before and after); there is no behavior difference for a test to redden. Verified the change is purely typological by confirming all 18 tests pass byte-identically to before. |
| **Commit** | `ee279e52` |
