...
| MAJOR-1 | `app/api/social/[platform]/disconnect/route.ts` now calls `getRegistry().get(platform).revokeAccessToken({socialAccountId})` **before** `deactivateSocialAccount`, wrapped in a `try/catch` that discards any failure (network error or `PROVIDER_NOT_CONFIGURED`) — local disconnect always completes. No new error code (L-1); no retry. Self-found follow-up: the build guide's own "bound the call, and state the bound" instruction was not met by `try/catch` alone (it discards a THROWN error, not a HANG) — added `signal: AbortSignal.timeout(REVOKE_TIMEOUT_MS)` (5000ms, named constant) to `twitter-provider.ts`'s revoke `fetch`. First production call site for `X_REVOKE_URL` — still the disclosed best-guess of ADR §16 item 10, now on a live path. `security-reviewer` reviewed both the ordering/never-blocks guarantee and the timeout addition (two attempts — first hit a session-wide rate limit and failed before producing output, no findings fabricated in its place; retried successfully after the reset). Confirmed: ordering correct, no meaningful TOCTOU, token never touches a URL/log, test coverage adequate. Two low-priority residual notes recorded, not acted on (out of MAJOR-1's scope, consistent with existing project patterns elsewhere): the two Supabase/RPC calls preceding the `fetch` inside `revokeAccessToken` have no independent timeout of their own, and an `instanceof AbortSignal` assertion wouldn't distinguish `AbortSignal.timeout` from a wired-but-never-triggered `AbortController.signal`. ADR §17.3 Table B and §17.4's `SOCIAL-REVOKE-NEVER-BLOCKS` row updated. | `disconnect.test.ts` — three new cases: (a) revoke called with the right platform and account, before `deactivateSocialAccount` (call-order asserted), demonstrated to REDDEN against the pre-fix route (revoke was never called at all); (b) a throwing revoke still returns 200 and deactivates, and (c) an unconfigured provider (`getRegistry().get` throws) still returns 200 and deactivates — both (b)/(c) mutation-tested by removing the `try/catch` (observed 500 instead of 200 on both, reverted). `twitter-provider.test.ts` — one new case asserting `init.signal` is an `AbortSignal`, mutation-tested by removing the `signal` option (observed `undefined`, reverted). | fd9b4094 |

Files touched by D3: `app/api/social/[platform]/disconnect/route.ts` (the revoke call + try/catch),
`app/api/social/[platform]/disconnect/disconnect.test.ts` (three new Tier-2 cases),
`lib/social/twitter-provider.ts` (`REVOKE_TIMEOUT_MS` + `AbortSignal.timeout` on the revoke fetch),
`lib/social/__tests__/twitter-provider.test.ts` (one new case),
`docs/decisions/0028-native-social-providers.md` (§17.3 Table B row + correction note, §17.4 row updated).

| MAJOR-3 | **A-11 — verified in-step, per §13's rule, before acting on the founder's "Yes."** LinkedIn's own "Getting Access to LinkedIn APIs" page lists exactly `profile`/`email`/`w_member_social` as the only permissions requiring no special approval; `r_member_postAnalytics` is not among them — it is review-gated (Community Management API), matching N2.1's "free via an approval form" note read correctly this time. Per the build guide's own instruction, this is the review-gated branch: **the scope is NOT added** to `PLATFORM_CONFIGS.linkedin.scopes`. Recorded as ADR 0028 §16 item 11 and §13 item 8 marked adjudicated with the date. `saves`/`clicks`/`reach`/`impressions` remain permanently null for LinkedIn until SOSH applies for and receives this permission. | None — no code changed (the review-gated branch ships no scope, so there is no positive assertion to write or redden). Verification is the vendor-doc citation itself, recorded in Appendix A item 15. | fe3c8d75 |
| adjud. 1 | **A-9′ recorded.** Dual identity (X, `posts.social_account_id`, `resolvePublishAccount`, the two-identity accounts surface) already shipped this session (A-6/A-8a) — confirmed by reading the code, not rebuilt. X delivers both Business and Founder fully at launch, already working. LinkedIn Company Page posting stays `coming_soon` (A-1's Meta pattern) while A-8's entity + Community Management API gate holds — unchanged and unreopened. No customer-facing copy says "member-only." A same-day founder challenge to A-5-prime's premise was raised and explicitly withdrawn — recorded as a dated one-line note, not as a finding, and A-5-prime/A-8 are unchanged. Ruling recorded in ADR §16 item 1 (as a dated addition, original text unedited) and `launch-checklist.md` §16a (checkbox now `[x]`). | None — this is a documentation-only ruling; the underlying capabilities (dual identity, X's two connections, `coming_soon` rendering) were already tested when they shipped earlier in this session. | fe3c8d75 |
| adjud. 2 | **A-10 recorded, narrowed.** The original ten-linked-posts-a-day figure was a worst-case ceiling presented as a forecast; rejected by the founder as unrealistic. Recomputed at a realistic 2-3 linked posts/day: ~$12-18/month against a €125 plan, under 15%, an ordinary COGS line. No ceiling set, no stated-open pricing decision opened, no code-level cap introduced. N2.1's 2M-vs-3M read-cap discrepancy carried forward as an unresolved factual gap. Recorded in ADR §16 item 6 (dated addition) and `launch-checklist.md` new §16b. | None — documentation-only; the ruling explicitly forbids any code change. | fe3c8d75 |

Files touched by D4: `docs/reviews/session-30-5-platform-verification.md` (new Appendix A item 15),
...
| MINOR-6 | **A-12, founder-confirmed.** `connection-status.ts` now returns `'disconnected'` (not `'expiring_soon'`) whenever `daysUntilExpiry < 0` — an already-expired token. Boundary is exclusive: exactly 0 (expiring today) is still `'expiring_soon'`. No sixth state — routes into the existing `disconnected` state, matching the ruling exactly. ADR §9.4 gains a one-line boundary note. | `connection-status.test.ts` — new case (token expired 3 days ago -> `disconnected`), demonstrated to REDDEN against the pre-fix code (returned `expiring_soon`); new boundary case pinning exactly-0 as still `expiring_soon`; the pre-existing 8-days-out case is the regression guard on the far side. | f6c5d156 |
| MINOR-7 | `PostCard.tsx`'s failure-surface copy now branches on `errorDetails.reason` (read from `ai_generation_metadata.publish_error.reason`, where `markPostFailed` already stores it) rather than `errorCode` alone — a `TOKEN_REVOKED` failure with `reason: 'account_ambiguous'` renders new i18n key `card.error.account_ambiguous` (added to en/pt/es simultaneously) instead of the reconnect copy. `lib/publishing/orchestrator.ts` has **zero diff** in this step (verified via `git diff --stat`) — no error code added, no control-flow change, matching L-1 exactly. | `PostCard.test.tsx` — two new cases: the ambiguous reason renders the disambiguation copy and NOT the reconnect copy, demonstrated to REDDEN against the pre-fix component (rendered the reconnect copy); and the regression case, an ordinary `TOKEN_REVOKED` with no ambiguous reason still renders the reconnect copy. | f6c5d156 |

Files touched by D6: `lib/social/connection-status.ts` (the `< 0` boundary), `lib/social/connection-status.test.ts`
(two new cases), `docs/decisions/0028-native-social-providers.md` (§9.4 boundary note), `components/posts/PostCard.tsx`
(reason-branching copy), `components/posts/PostCard.test.tsx` (two new cases), `i18n/en/posts.json`,
`i18n/pt/posts.json`, `i18n/es/posts.json` (new `card.error.account_ambiguous` key, all three simultaneously).

| MINOR-5 | `provider-contract.test.ts`'s exemption comment re-tensed: "until N2.10 makes the registry overrides-only" / "the moment N2.10 adds..." rewritten to state the shipped state (N2.10 landed inside the reviewed range), with line ranges re-cited against HEAD (`registry.ts:16-34`, `:44-54`, matching the Reviewer's own citations). The exemption's substantive justification is unchanged. | None — comment-only, verified by re-reading `registry.ts` at the cited lines. | b816bfa2 |
| NIT-1 | `provider_unavailable` removed entirely from `i18n/{en,pt,es}/common.json` after confirming by grep that no route (only an unrelated email-provider string of the same name exists elsewhere) can ever emit `error=provider_unavailable` — it was already excluded from `resolve-banner.ts`'s `ERROR_KEYS`. The presence-pinning assertion in `accounts-i18n.test.ts` is replaced with an absence-pinning regression guard (the one test deletion this pass authorises, paired with removing the string). `resolve-banner.ts`'s comment updated to past tense. | `accounts-i18n.test.ts` — the new case asserts `provider_unavailable` is NOT a key in any of the three locales; `resolve-banner.test.ts`'s pre-existing "does not include provider_unavailable" cases are untouched and still pass. | b816bfa2 |
| NIT-2 | ADR §16 items 7 and 8 ("Re-authorisation is unavoidable..." and "§14 may be empty...") had their number LABELS swapped so they read in ascending order — neither item's text content changed. A dated note on item 8 records the swap. The one build-guide cross-reference to the old "§16 item 8" (re-authorisation) re-pointed to "§16 item 7". | None — documentation-only, verified by re-reading the renumbered items and grepping for any remaining stale cross-reference (none found). | b816bfa2 |
| NIT-3 | **Discovery, not a routine fix.** Verified "six credentials"→four (`config.ts:536-546`, exactly `LINKEDIN_CLIENT_ID/SECRET`, `X_CLIENT_ID/SECRET`) and "four states"→five (`connection-status.ts:5`) — both corrected in the build guide. Then verified the "eight rows"→seven claim by `git log -- docs/launch-checklist.md` plus direct enumeration (`grep -n '^\- \['`, lines 456-463): §16 has held **exactly eight rows since N2.11 (`231fee86`)**, unchanged. **The build guide's original "eight" was correct all along; ADR 0028's own "corrected to seven" claim (§8.3, §12) was the actual miscount**, and the N3 Reviewer's verification table trusted that ADR claim without an independent recount. §5's "all eight rows checked" needed no fix — it was already right. Corrected: the D7 instruction paragraph itself (which repeated the wrong premise), the NIT-3 summary-table row, and ADR §8.3/§12 (dated correction notes, original wrong claims left visible per this ADR's append-only convention for already-Accepted text). | None — every count verified by `grep`/file read/`git log`, evidence quoted above and in the commit body. | b816bfa2 |

Files touched by D7: `lib/social/__tests__/provider-contract.test.ts` (comment re-tense), `i18n/en/common.json`,
`i18n/pt/common.json`, `i18n/es/common.json` (key removal), `app/[locale]/(dashboard)/settings/accounts/accounts-i18n.test.ts`
(assertion swap), `app/[locale]/(dashboard)/settings/accounts/resolve-banner.ts` (comment), `docs/decisions/0028-native-social-providers.md`
(§16 renumber, §8.3/§12 correction notes), `docs/build-guide/session-30-5.md` (arithmetic sweep + the NIT-3 discovery correction).

| BLOCKER-2 | `database-reviewer` diagnosed the cause beyond the Reviewer's own read: `OOMKilled: false` on the Postgres container does not rule out memory pressure — Postgres is multi-process, and a cgroup kill of a CHILD backend (a GoTrue-driven connection, a checkpointer) never sets the tracked-PID-1 flag, but Postgres's crash-recovery design still terminates every backend and restarts into recovery, producing exactly the observed `57P03` signature. Fix attempted: `supabase/config.toml`'s `shared_buffers` lowered 256MB→128MB and `maintenance_work_mem` 128MB→64MB (freeing headroom for GoTrue's bcrypt spikes instead of competing with them); `db-tests.yml` gains a backgrounded live memory/cgroup-pressure watch uploaded **unconditionally**, not `if: failure()`, since a mid-run cgroup kill may never trip that trap, plus a cgroup-memory read added to the existing failure-diagnostics step. **As of this commit no green db-tests run exists at a commit in this corrected range** — D9 is where the corrected range is pushed and CI-verified, per the build guide's own ordering rationale ("D8 is deliberately late... CI runs LAST (D9)"). `SOCIAL-VAULT-UPDATE-SECRET` and `SOCIAL-DUAL-IDENTITY-SCHEMA` are marked `AUTHORED-NOT-EXECUTED` **explicitly in ADR §17.4's table** (not a nearby paragraph) as the honest current state, with the three red run URLs and the diagnosed cause. No retry into a false green attempted; no skip-guard relaxed; no collection reduced; the promotion tally is untouched. | `npm run test:db` could not be run locally (Docker daemon unreachable — same limitation the N3 Reviewer hit); the YAML was validated with `js-yaml` for syntax only. Evidence of record is the CI log at D9. | 933a335c |
| NIT-4 | ADR §17.4's db-tests paragraph and `docs/backlog.md`'s `30.5-DBTESTS-READINESS-RACE` entry both named only two invisible suites; both now name all **seven**: `campaigns-social-accounts-role-policies`, `governed-memory-recency-column`, `learning-report-orphans`, `performance-memory-candidates-expiry`, `post-ai-originals-latest-per-post`, `reissue-invite`, `signals3-triage-atomic`. Whether they are new to this range is recorded as **unknown** (no green baseline exists to establish it), not assumed benign. | None — documentation, the seven names are quoted verbatim from the CI log the Reviewer already cited. | 933a335c |

Files touched by D8: `.github/workflows/db-tests.yml` (knob value + live memory-watch + cgroup diagnostics),
`supabase/config.toml` (`shared_buffers`, `maintenance_work_mem`), `docs/decisions/0028-native-social-providers.md`
(§17.4 correction note + two table rows marked `AUTHORED-NOT-EXECUTED`), `docs/backlog.md` (readiness-race row updated).

### D9 close-out

**Pushed and re-verified at the corrected range head, `933a335c`.**

- **`app-tests` — GREEN.** [run 33998672886](https://github.com/tcr430/SOSH/actions/runs/33998672886).
  Every constraint touched by D1–D8 (including the three new/renamed ones — `SOCIAL-PINNED-ACCOUNT-TENANT-CHECKED`,
  the `revokeAccessToken` call-site row in Table B, `finiteRetryAfterSeconds`) executed green.
- **`db-tests` — RED a FOURTH consecutive time.** [run 33998672960](https://github.com/tcr430/SOSH/actions/runs/33998672960).
  D8's memory-tuning fix (`shared_buffers`/`maintenance_work_mem` lowered) was pushed and re-tested; it did
  **not** produce a green run. The skip-guard output, quoted verbatim from the log:
  `skip-guard: /home/runner/work/SOSH/SOSH/supabase/__tests__/post-ai-originals-latest-per-post.test.ts — every test is skipped (invisible — not covered)`,
  `skip-guard: /home/runner/work/SOSH/SOSH/supabase/__tests__/signals3-triage-atomic.test.ts — every test is skipped (invisible — not covered)`,
  `skip-guard: 3 failing test(s) — a RED suite must fail the job, never be swallowed`, naming the same three
  `vault-update-secret.test.ts` permission cases as every prior red. **New evidence changes the diagnosis**:
  `docker inspect` on `supabase_db_SOSH` shows `ExitCode=0`, healthy host memory (`free -h`: 7.8Gi free) —
  no OOM-shaped signature at all this time. The container log instead shows
  `LOG: server process (PID 362) was terminated by signal 11: Segmentation fault`, immediately followed by
  the same `terminating any other active server processes` / `FATAL: the database system is in recovery
  mode` sequence as the prior three reds, coinciding with `vault_update_secret` RPC calls across all four
  reds observed. A SIGSEGV in a Postgres backend is not a resource-exhaustion signature — the memory-tuning
  fix addressed the wrong theory. `docs/decisions/0028-native-social-providers.md` §17.4 and
  `docs/backlog.md`'s `30.5-DBTESTS-READINESS-RACE` entry are both updated with this sharper diagnosis in
  this same D9 commit. `SOCIAL-VAULT-UPDATE-SECRET` and `SOCIAL-DUAL-IDENTITY-SCHEMA` remain
  `AUTHORED-NOT-EXECUTED` — now with a fourth red data point, which strengthens rather than weakens the
  case that this is a genuine, reproducible infrastructure defect (a Postgres/`pgsodium` crash) and not
  transient CI noise.

**A stale citation inside the Reviewer's own original text, noted here per this pass's own rule (never
edited above the appendix):** line 341 of this file reads *"§16 item 8 already plans one forced
re-authorisation"* — at the time the Reviewer wrote it, the re-authorisation item was numbered 8. D7's
NIT-2 fix swapped the number labels on ADR §16's items 7 and 8 (transposed in the original numbering), so
that same content is now **§16 item 7**. The Reviewer's sentence is left exactly as written; a reader
following it to "§16 item 8" today will find the *other* item (`§14 may be empty at merge`) instead — this
paragraph is the correction.

**Ordering rationale, recorded so the sequence does not read as arbitrary (restating the build guide's own
§4 rationale, verified against what was actually done):** D0 landed the reviewer report before any
resolution row was appended, so the append-only diff proves nothing above the appendix moved. D1 fixed the
platform-fact provenance defect (BLOCKER-1/MINOR-1/MINOR-2) before any other step could add a new platform
fact of its own (D3's revoke URL, D4's LinkedIn scope) — fixing provenance discipline after adding more
facts to it would have meant auditing twice. D2 (MAJOR-2, cross-tenant publish) landed first among the code
fixes because it is the only finding with a cross-tenant publish as its failure mode. D3 (MAJOR-1) followed
because it also touches account resolution and consumes D1's `X_REVOKE_URL` output as its first production
call site. D4 grouped the three founder-ruling items so the one line of code among them (A-11) wasn't
hidden inside two documentation-only commits. D5 grouped the two "guarantee weaker than its name" findings
(MINOR-3/MINOR-4) as the same shape of defect. D6 grouped the two user-facing findings as one UX surface and
one i18n sweep. D7 was the documentation-and-comment truth sweep, run before D8 so NIT-3's arithmetic
verification wasn't itself working from a numbering D7 hadn't yet fixed. D8 was deliberately late — its
remedy (a green `db-tests` run, or an honest non-execution marking) could only be attempted once every
migration-touching change in the pass had landed. D9 ran last because CI verification of the corrected
range is what makes every earlier step's citation true rather than merely reworded — and it did exactly
that: it turned D8's "pending" marking into a confirmed, sharper diagnosis rather than a rubber stamp.

**Row-count reconciliation, checked rather than assumed:** the report's closing tally line reads *"16
findings (2 BLOCKER, 3 MAJOR, 7 MINOR, 4 NIT)"*. Counting this appendix's own rows: BLOCKER-1, BLOCKER-2 (2)
+ MAJOR-1, MAJOR-2, MAJOR-3 (3) + MINOR-1 through MINOR-7 (7) + NIT-1 through NIT-4 (4) = **16**, plus
adjud. 1 and adjud. 2 = **18 rows total**, matching the table above exactly. Unlike Session 29, there was
nothing to argue here — the report's own tally was correct, and this pass's job was to verify that, not
assume it.

Files touched by D9: `docs/decisions/0028-native-social-providers.md` (the fourth-red correction note,
sharper diagnosis, and the re-verified `app-tests` citation), `docs/backlog.md` (sharper diagnosis),
`docs/reviews/session-30-5-reviewer.md` (this close-out section, the appendix header finalised, all eight
SHA placeholders filled in), `.wolf/anatomy.md`, `.wolf/memory.md`, `.wolf/cerebrum.md`.
